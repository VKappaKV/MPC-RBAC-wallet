...
export async function prepareEthSigning(
  ethAmount: string,
...
export async function sendTxToEthereumBaseSepolia(
  sign_id: string,
  ...
  const ethTxHash = await broadcastTransaction(
    unsignedTx,
    new Uint8Array(rawSignature),
    ethereumAddr,
    ethClient as PublicClient,
  );

  console.log(
    `Transaction corretly broadcasted to base sepolia with Tx ID: ${ethTxHash}`,
  );
}
...
async function getDwalletPubKey(
  dwallet: DWalletWithState<"Active">,
  curve: Curve,
) {
...
async function broadcastTransaction(
  ...
  const r = `0x${Buffer.from(signature.slice(0, 32)).toString("hex")}` as Hex;

  const s = `0x${Buffer.from(signature.slice(32, 64)).toString("hex")}` as Hex;

  let signedTx: Hex | null = null;
  for (const yParity of [0, 1] as const) {
    const reconstructedTransaction = serializeTransaction(unsignedTx, {
      r,
      s,
      yParity,
    });
  ...
  if (!signedTx) {
    throw new Error("error with v signature");
  }
